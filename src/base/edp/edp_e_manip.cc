...
bool manip_effector::compute_servo_joints_and_frame(void) {
	...
	try {
		...
		while (!sb) {
			std::cerr << "Race condition detected! (" << __FILE__ << ":"
					<< __LINE__ << ")" << std::endl;
			usleep(1000);
		}

		lib::MotorArray servo_desired_motor_pos(
				sb->command.parameters.move.servo_desired_motor_pos_new,
				number_of_servos);

		lib::JointArray servo_desired_joints(number_of_servos);

		get_current_kinematic_model()->mp2i_transform(servo_desired_motor_pos,
				servo_desired_joints);
		...
		servo_real_kartez_vel = (servo_real_kartez_pos
				- servo_prev_real_kartez_pos) / (lib::EDP_STEP);
		servo_real_kartez_acc = (servo_real_kartez_vel
				- servo_prev_real_kartez_vel) / (lib::EDP_STEP);
		servo_desired_kartez_vel = (servo_desired_kartez_pos
				- servo_prev_desired_kartez_pos) / (lib::EDP_STEP);
		...
		// scope-locked reader data update
		{
			...
			servo_real_kartez_vel.to_table(
					rb_obj->step_data.real_cartesian_vel);
			servo_real_kartez_acc.to_table(
					rb_obj->step_data.real_cartesian_acc);
			servo_desired_kartez_pos.to_table(
					rb_obj->step_data.desired_cartesian_position);
			servo_desired_kartez_vel.to_table(
					rb_obj->step_data.desired_cartesian_vel);
		}
		...
		if (vs != NULL) {
			...
			if ((is_synchronised())
					&& (!(force_sensor_post_synchro_configuration))) {
		...
		if (imu_sen != NULL) {
			...
			if ((is_synchronised())
					&& (!(imu_sensor_post_synchro_configuration))) {
...
/*--------------------------------------------------------------------------*/
manip_effector::manip_effector(shell &_shell,
		const lib::robot_name_t & l_robot_name, lib::c_buffer & c_buffer_ref,
		lib::r_buffer & r_buffer_ref) :
...
void manip_effector::get_arm_position_with_force_and_sb(bool read_hardware,
	...
	reply.arm.pf_def.force_xyz_torque_xyz = ft_tr_inv_tool_matrix
...
void manip_effector::compute_base_pos_xyz_rot_xyz_vector(
	...
	case lib::ABSOLUTE:
		...
		case lib::JOINT:
			...
			break;
		case lib::MOTOR:
			get_current_kinematic_model()->mp2i_transform(motor_arm_coordinates,
					tmp_joints);
			get_current_kinematic_model()->i2e_transform(tmp_joints,
					goal_frame);
	...
	case lib::PF_VELOCITY:
		base_pos_xyz_rot_xyz_vector = lib::Xyz_Angle_Axis_vector(
...
void manip_effector::pose_force_torque_at_frame_move(
	...
	// macrostep iteration
	iterate_macrostep(begining_joints, begining_end_effector_frame, instruction,
			base_pos_xyz_rot_xyz_vector);
}
/*--------------------------------------------------------------------------*/
...
void manip_effector::set_robot_model(const lib::c_buffer &instruction) {
	...
	{
		const lib::Homog_matrix & hm =
				instruction.robot_model.tool_frame_def.tool_frame;

		if (!(hm.is_valid())) {
			BOOST_THROW_EXCEPTION(
					nfe_2() << mrrocpp_error0(INVALID_HOMOGENEOUS_MATRIX));
		}
...
void manip_effector::compute_frame(const lib::c_buffer &instruction) {
	...
	if ((value_in_step_no <= 0) || (motion_steps <= 0)
			|| (value_in_step_no > motion_steps + 1)) {
		BOOST_THROW_EXCEPTION(
				nfe_2() << mrrocpp_error0(INVALID_MOTION_PARAMETERS));
...
void manip_effector::get_controller_state(const lib::c_buffer &instruction) {
