...
#include "ecp_t_pid_tuning_with_settings_list_pb_eih.h"
...
char ecp_t_pid_tuning_with_settings_list_pb_eih::config_section_name[128] = { "[pid_tuning_with_settings_list_pb_eih]" };

ecp_t_pid_tuning_with_settings_list_pb_eih::ecp_t_pid_tuning_with_settings_list_pb_eih(mrrocpp::lib::configurator& config) :
	common::task::task(config)
{
	try{
		...
		if(robot_name == lib::irp6p_m::ROBOT_NAME){
			...
			throw std::runtime_error("ecp_t_pid_tuning_with_settings_list_pb_eih option robot_name in config file has unknown value: " + robot_name);
		}
		...
		step_axis = config.value<int>("step_axis", config_section_name);
		...
		log_dbg("ecp_t_pid_tuning_with_settings_list_pb_eih::ecp_t_pid_tuning_with_settings_list_pb_eih(): 3\n");
		sm = boost::shared_ptr <single_visual_servo_manager> (new single_visual_servo_manager(*this, config_section_name, vs));
		log_dbg("ecp_t_pid_tuning_with_settings_list_pb_eih::ecp_t_pid_tuning_with_settings_list_pb_eih(): 4\n");
		sm->add_position_constraint(cube);

		sm->add_termination_condition(obj_reached_term_cond);
		sm->add_termination_condition(timeout_term_cond);
		log_dbg("ecp_t_pid_tuning_with_settings_list_pb_eih: configuring visual_servo_manager\n");
		sm->configure();
	}catch(std::exception& ex){
		sr_ecp_msg->message(lib::FATAL_ERROR, string("ERROR in ecp_t_pid_tuning_with_settings_list_pb_eih: ") + ex.what());
		throw ex;
	}
	log_dbg("ecp_t_pid_tuning_with_settings_list_pb_eih: initialization completed.\n");
}

void ecp_t_pid_tuning_with_settings_list_pb_eih::load_regulator_settings_from_config()
{
	for(int i=0;;++i){
	...
	log_dbg("%d regulator settings loaded.", regulator_settings.size());
}

void ecp_t_pid_tuning_with_settings_list_pb_eih::main_task_algorithm(void)
{
	char txt[256];
//	newsmooth_gen->set_debug(true);
	while (1) {
		...
		if (mp_2_ecp_next_state_string == mrrocpp::ecp_mp::generator::ECP_GEN_VISUAL_SERVO_TEST) {
			...
			starting_position_hm(step_axis, 3) += step_distance;
			starting_position_hm.get_xyz_angle_axis(xyz_aa);
			xyz_aa.to_vector(starting_position);

			// for every setting in the settings list
			for(int i=0; i<regulator_settings.size(); ++i){
				for(int probe_number=0; probe_number<number_of_probes; ++probe_number){
					sprintf(txt, "Running VS with setting %d... (probe %d)", i, probe_number);
					sr_ecp_msg->message(txt);
					log("%s", txt);

					apply_regulator_settings(i);

					run_vs();

					move_files(i, probe_number);
				}
			}

			sr_ecp_msg->message("Finished");
		} else {
			log("ecp_t_pid_tuning_with_settings_list_pb_eih::main_task_algorithm(void) mp_2_ecp_next_state_string: \"%s\"\n", mp_2_ecp_next_state_string.c_str());
		}
	}
...
void ecp_t_pid_tuning_with_settings_list_pb_eih::apply_regulator_settings(unsigned int idx)
{
...
void ecp_t_pid_tuning_with_settings_list_pb_eih::run_vs()
{
...
void ecp_t_pid_tuning_with_settings_list_pb_eih::move_files(int setting_idx, int probe_idx)
{
	char destDir[256];
	sprintf(destDir, "%s/%03d_%03d/", destination_dir.c_str(), setting_idx, probe_idx);
...
{
	return new ecp_t_pid_tuning_with_settings_list_pb_eih(config);
}
