/*
 * ecp_t_przestawianie.h
 *
 *  Created on: Apr 21, 2010
 *      Author: mboryn
 */

#ifndef ecp_t_przestawianie_H_
#define ecp_t_przestawianie_H_
 ...
 *
 */
class ecp_t_przestawianie : public mrrocpp::ecp::common::task::task
{
public:
	ecp_t_przestawianie(mrrocpp::lib::configurator& config);
...
private:
	...
	std::vector<double> object_reached_position;
	lib::Homog_matrix object_reached_position_hm;
	...
	void iteration(int setting_idx, int axis_idx, int direction, int probe_idx);

	void run_vs();

	void move_files(int setting_idx, int axis_idx, int direction, int probe_idx);
